...
from app.schemas.generation_jobs import GenerationJobStatusOut, TextGenerationJobOut
from app.schemas.poi_enhancements import InfoTextBatchRequest, InfoTextStatusResponse
...
async def get_info_text(
    ...
    topic_obj = await get_information_topic_by_name(
        session, topic
    )  # Can we do this in the service layer?
    if not topic_obj:
