...
from app.schemas.poi_enhancements import POIImageCreate, POIImageOut, POIImageUpdate
...
async def get_images_for_poi(poi_id: int, session=Depends(get_session)):
    ...
    return [
        serialize_poi_image(img, base_image_url=settings.BASE_IMAGE_URL)
        for img in images
    ]
...
async def create_image(image: POIImageCreate, session=Depends(get_session)):
    ...
    return serialize_poi_image(created, base_image_url=settings.BASE_IMAGE_URL)


# ADMIN ENDPOINT!!!
@router.patch("/images/{image_id}", response_model=POIImageOut)
async def update_image(
    image_id: int, image: POIImageUpdate, session=Depends(get_session)
):
    updated = await update_poi_image(
        session, image_id, **image.dict(exclude_unset=True)
    )
    if not updated:
        raise HTTPException(status_code=404, detail="Image not found")
    return serialize_poi_image(updated, base_image_url=settings.BASE_IMAGE_URL)


# ADMIN ENDPOINT!!!
@router.delete("/images/{image_id}", status_code=204)
async def delete_image(image_id: int, session=Depends(get_session)):
    img = await get_poi_image_by_id(session, image_id)
    if not img:
        raise HTTPException(status_code=404, detail="Image not found")
