...
async def update_info_text(info_id: int, info: POIInfoTextUpdate, session=Depends(get_session)):
    updated = await update_poi_info_text(session, info_id, **info.dict(exclude_unset=True))
    if not updated:
