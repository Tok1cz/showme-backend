...
class POIInfoText(Base):
    ...
    text_length: Mapped[TextLength] = mapped_column(
        Enum(TextLength, name="text_length"), nullable=False, default=TextLength.medium
    )
    ...
    topic_id: Mapped[int] = mapped_column(
        ForeignKey("information_topics.id"), nullable=True
    )
    style_id: Mapped[int] = mapped_column(
        ForeignKey("information_styles.id"), nullable=True
    )
    audio_id: Mapped[int] = mapped_column(ForeignKey("poi_audio.id"), nullable=True)
    task_id: Mapped[UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("text_generation_jobs.task_id"), nullable=True
    )
    ...
    audio = relationship(
        "POIAudio", lazy="joined", uselist=False, foreign_keys=[audio_id]
    )
    text_generation_job = relationship(
        "TextGenerationJob", lazy="joined", uselist=False, foreign_keys=[task_id]
    )
