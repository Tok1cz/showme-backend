...
class POIAudio(Base):
    ...
    quality: Mapped[AudioQuality] = mapped_column(
        Enum(AudioQuality, name="audio_quality"),
        nullable=False,
        default=AudioQuality.medium,
    )
    length: Mapped[AudioLength] = mapped_column(
        Enum(AudioLength, name="audio_length"),
        nullable=False,
        default=AudioLength.medium,
    )
    ...
    task_id: Mapped[UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("audio_generation_jobs.task_id"), nullable=True
    )

    # Relationships to helper/reference tables
    style = relationship("InformationStyle", lazy="joined")
    audio_generation_job = relationship(
        "AudioGenerationJob", lazy="joined", uselist=False, foreign_keys=[task_id]
    )
