...
async def get_aspect_id(session: AsyncSession, aspect_name: Optional[str]) -> Optional[int]:
    if not aspect_name:
        return None
    result = await session.execute(
...
async def get_images_for_poi_ids(session: AsyncSession, ids: list[int]) -> list[POIImage]:
    stmt = select(POIImage).where(POIImage.poi_id.in_(ids)).options(
        joinedload(POIImage.style), joinedload(POIImage.aspect)
    )
    result = await session.execute(stmt)
    return result.scalars().all()
