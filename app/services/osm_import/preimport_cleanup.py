...
def preimport_cleanup(db_url: str = None, temp_prefix: str = "planet_osm_new"):
    ...
    with engine.begin() as conn:
        ...
        for conname, tablename in constraints:
            ...
            try:
                conn.execute(text(f'ALTER TABLE {tablename} DROP CONSTRAINT IF EXISTS {conname} CASCADE'))
            except Exception as e:
                logger.warning("Failed to drop constraint %s on %s: %s", conname, tablename, e)
        ...
        for fname, argtypes in functions:
            ...
            try:
                ...
                conn.execute(text(f'DROP FUNCTION IF EXISTS {fname}({argtypes}) CASCADE'))
            except Exception as e:
        ...
        for tname, relname in triggers:
            ...
            try:
                conn.execute(text(f'ALTER TABLE {relname} DROP TRIGGER IF EXISTS {tname}'))
            except Exception as e:
