...
def validate_imported_tables(
    ...
    with engine.connect() as conn:
        for t in required_tables:
            ...
            # Check table exists
            exists = conn.execute(
                text("SELECT to_regclass(:tn) IS NOT NULL"),
                {"tn": tablename}
            ).scalar()
            if not exists:
            ...
            if count is None or count < min_expected_rows:
                logger.warning(
                    ...
                    tablename, count, min_expected_rows
                )
            results[tablename] = count

    logger.info("Validation complete for tables with prefix '%s': %s", table_prefix, results)
    return results
